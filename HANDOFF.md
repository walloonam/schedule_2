...
4. 개발 서버 실행: `npm run dev -- --hostname 127.0.0.1 --port 3000`
5. 접속: `http://127.0.0.1:3000`
...
"이 저장소의 `docs/agents/README.md`와 `docs/agents/task-template.md`를 먼저 읽고, 고정 팀(Kuhn/Bacon/Beauvoir/Dirac)만 사용해서 작업해줘. 작업 끝나면 dev 서버 재기동 후 `/`와 `/briefing` 상태코드 확인까지 해줘."
