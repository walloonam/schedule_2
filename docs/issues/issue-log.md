...
실제 GitHub issue 번호는 `#1`부터 `#20`까지 생성되었다.
...
- [ ] `[NLP-04] 자연어 입력에 기간/종료시각 지정 지원` (`#20`)
  - 시작 시각만이 아니라 `오후 2시부터 4시까지` 같은 종료시각/기간 표현을 자연스럽게 처리하는 작업이 남아 있다.

## 사용법

- 새 작업이 끝나면 `Done`에 체크를 옮긴다.
- 다시 해야 하는 일이 생기면 `Open`으로 분리한다.
- `Open` 항목은 아직 `기대 결과` 단계로 읽고, 완료되면 실제 `결과`로 해석한다.
